...
// Try to load @verdikta/common, fallback to static data if not available
let classMap = null;
...
try {
    const verdiktaCommon = require('@verdikta/common');
    classMap = verdiktaCommon.classMap;
} catch (error) {
    // @verdikta/common not available, use fallback data
    useFallback = true;
    
    // Fallback ClassID data - matches actual @verdikta/common data as of latest version
    const fallbackClasses = [
        {
            id: 128,
            name: 'OpenAI & Anthropic Core',
            status: 'ACTIVE',
            models: [
                { provider: 'openai', model: 'gpt-5-2025-08-07', supports_images: true, supported_file_types: ['pdf', 'images'] },
                { provider: 'openai', model: 'gpt-5-mini-2025-08-07', supports_images: true, supported_file_types: ['pdf', 'images'] },
                { provider: 'openai', model: 'gpt-5-nano-2025-08-07', supports_images: true, supported_file_types: ['pdf', 'images'] },
                { provider: 'anthropic', model: 'claude-sonnet-4-20250514', supports_images: true, supported_file_types: ['pdf', 'images'] },
                { provider: 'anthropic', model: 'claude-3-7-sonnet-20250219', supports_images: true, supported_file_types: ['pdf', 'images'] },
                { provider: 'anthropic', model: 'claude-3-5-haiku-20241022', supports_images: true, supported_file_types: ['pdf', 'images'] }
            ],
            limits: { max_outcomes: 10, max_panel_size: 5 }
        },
        {
            id: 129,
            name: 'Open-Source Local (Ollama)',
            status: 'ACTIVE',
            models: [
                { provider: 'ollama', model: 'llama3.1:8b', supports_images: false, supported_file_types: ['text'] },
                { provider: 'ollama', model: 'llava:7b', supports_images: true, supported_file_types: ['images', 'text'] },
                { provider: 'ollama', model: 'deepseek-r1:8b', supports_images: false, supported_file_types: ['text'] },
                { provider: 'ollama', model: 'qwen3:8b', supports_images: false, supported_file_types: ['text'] },
                { provider: 'ollama', model: 'gemma3n:e4b', supports_images: false, supported_file_types: ['text'] }
            ],
            limits: { max_outcomes: 10, max_panel_size: 3 }
        },
        {
            id: 130,
            name: 'OSS via Hyperbolic API (reserved)',
            status: 'EMPTY',
            models: [],
            limits: { max_outcomes: 10, max_panel_size: 3 },
            note: 'Requires Hyperbolic API key when active (not Ollama)'
        }
    ];
    
    // Mock classMap interface for fallback
    classMap = {
        listClasses: () => fallbackClasses.map(cls => ({ id: cls.id })),
        getClass: (id) => fallbackClasses.find(cls => cls.id === id)
    };
}
