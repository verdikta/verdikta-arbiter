...
if ! ask_yes_no "Register with dispatcher?" "" VA_REGISTER_ORACLE y; then
...
prompt_value "Aggregator address (0x...): " NEW_AGGREGATOR_ADDRESS VA_AGGREGATOR_ADDRESS
    ...
    if [ "$new_lc" = "$stored_lc" ]; then
        ...
        case "$active" in
            ...
            *)
                ...
                if ! ask_yes_no "Do you want to continue anyway?" "" VA_REGISTER_CONTINUE_IF_ALREADY n; then
...
prompt_value "Classes IDs [$classes_default]: " NEW_CLASSES_INPUT VA_CLASS_IDS
NEW_CLASSES_INPUT=${NEW_CLASSES_INPUT:-$classes_default}
NEW_CLASSES_INPUT="$(echo "$NEW_CLASSES_INPUT" | tr ',' ' ' | tr -s ' ' | sed 's/^ *//; s/ *$//')"  # accept comma-separated too
...
echo "$REGISTER_CMD"
echo ""
if ! ask_yes_no "Proceed with registration?" "" "" y; then
    echo -e "${YELLOW}Oracle registration cancelled.${NC}"
    exit 0
fi
