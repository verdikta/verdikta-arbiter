#!/bin/bash
#
# Verdikta Arbiter Doctor — REMOTE (read-only) variant
#
# This is the portable read-only variant of arbiter-doctor.sh suitable for
# scp-ing to any Verdikta Arbiter installation. It performs the same checks as
# the canonical doctor but refuses --fix and --collect: nothing is written or
# changed on the target host.
#
...
# REMOTE variant: force read-only mode. Any destructive flag is rejected.
REMOTE_READONLY=1
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --fix)
            echo "remote-doctor.sh refuses --fix; use the in-install arbiter-doctor.sh for remediation." >&2
            exit 64 ;;
        --collect)
            echo "remote-doctor.sh refuses --collect; use the in-install arbiter-doctor.sh for log collection." >&2
            exit 64
            # unreachable below — preserved for diff parity:
            MODE="collect"
            shift
            if [ $# -gt 0 ] && [[ "$1" != --* ]]; then
                COLLECT_OUT="$1"; shift
...
if [ "$OUTPUT" = "human" ]; then
    echo -e "${C_BOLD}========================================${C_NC}"
    echo -e "${C_BOLD}Verdikta Arbiter Doctor (REMOTE, read-only)${C_NC}"
    echo -e "${C_BOLD}========================================${C_NC}"
    echo "host:     $(hostname)"
