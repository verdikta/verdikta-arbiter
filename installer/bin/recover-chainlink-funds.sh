...
# Verdikta Arbiter Node - Chainlink Fund Recovery Script
# Recovers excess ETH and accumulated LINK tokens from Chainlink keys back to the owner's wallet
...
usage() {
    ...
    echo ""
    echo "Recover excess ETH and accumulated LINK tokens from Chainlink keys back to owner's wallet"
    echo ""
    echo "Options:"
...
if [ "$RECOVER_BOTH" = "true" ]; then
    RECOVER_ETH=true
    RECOVER_LINK=true
elif [ "$RECOVER_ETH" = "false" ] && [ "$RECOVER_LINK" = "false" ]; then
...
# Function to get private key for a Chainlink key using the key management script
get_chainlink_private_key() {
    ...
    if [ -z "$private_key" ]; then
        echo "ERROR: Empty private key returned for $key_address"
        return 1
    fi
...
echo -e "${BLUE}"
echo "============================================================"
echo "  Verdikta Arbiter - Chainlink Fund Recovery"
echo "============================================================"
echo -e "${NC}"
...
if [ "$RECOVER_ETH" = "false" ] && [ "$RECOVER_LINK" = "false" ]; then
...
for KEY_ADDRESS in "${KEY_ADDRESSES[@]}"; do
    ...
    if [ "$RECOVER_LINK" = "true" ] && (( $(echo "${KEY_LINK_RECOVERABLE[$KEY_ADDRESS]:-0} > 0" | bc -l) )); then
        TRANSACTIONS_NEEDED=$((TRANSACTIONS_NEEDED + 1))
    fi
done
...
# Final confirmation
if [ "$FORCE_RECOVERY" = "false" ] && [ "$INTERACTIVE" = "true" ]; then
