    ...
    return new Promise((resolve, reject) => {
      ...
      operation.attempt(async (currentAttempt) => {
        try {
              ...
              for (const file of query.additional) {
                try {
                  ...
                  if (file.path) {
                    ...
                    logger.info(`Added file from path to attachments: ${file.path}, type: ${mimeType}, format: ${typeof attachment === 'object' ? attachment.type : 'string'}`);
                  }
                } catch (fileError) {
                  logger.warn(`Failed to read file:`, fileError.message);
                  continue;
                }
              }
              ...
              for (const file of manifest.support) {
                try {
                  if (file.path) {
                    ...
                    logger.info(`Detected MIME type for support file: ${mimeType}`);
                    
                    const attachment = encodeAttachment(fileData, mimeType);
                    attachments.push(attachment);
                    logger.info(`Added support file to attachments: ${file.name || file.path}, size: ${fileData.length} bytes, type: ${mimeType}, format: ${typeof attachment === 'object' ? attachment.type : 'string'}`);
                  }
                } catch (fileError) {
                  logger.error(`Failed to read support file:`, fileError);
                  continue;
                }
              }
          ...
          if (isProviderError) {
            logger.error('Provider error encountered. Stopping retries.', { providerError: providerErrorMessage });
