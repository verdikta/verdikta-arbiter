...
| P0-3 | Run secret hygiene pass and rotate exposed credentials | Security + DevOps | Todo | All active credentials used during development are rotated; repository scan reports no verified leaked secrets in tracked files; remediation log captured in issue/PR. | Automated scan found no hardcoded secrets in tracked files. Manual credential rotation still required. |
